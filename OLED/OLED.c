...
#define OLED_Pin_SCL 2
#define OLED_Pin_SDA 4

#define OLED_SCL_w(x) digitalWrite(OLED_Pin_SCL, x);
#define OLED_SDA_w(x) digitalWrite(OLED_Pin_SDA, x);
...
OLED_DisplayModeSettings OLED_DisplayMode;
...
void OLED_I2C_Init(void)
{
	...
	pinMode(OLED_Pin_SCL, OUTPUT);
	pinMode(OLED_Pin_SDA, OUTPUT);
...
void OLED_Dot(uint8_t ScreenX, uint8_t ScreenY, OLED_ColorTypeDef Color)
{
	...
	if (OLED_DisplayMode == Default) // TODO
	{
...
void OLED_FilletMatrix(uint8_t ScreenX1, uint8_t ScreenY1, uint8_t ScreenX2, uint8_t ScreenY2, uint8_t Radius, uint8_t Fill, OLED_ColorTypeDef Color)
{
	...
	if (Fill == 1)
	{
		OLED_Matrix(ScreenX1 + Radius, ScreenY1, ScreenX2 - Radius, ScreenY1 + Radius, 1, Color);
		OLED_Matrix(ScreenX1, ScreenY1 + Radius, ScreenX2, ScreenY2 - Radius, 1, Color);
		OLED_Matrix(ScreenX1 + Radius, ScreenY2 - Radius, ScreenX2 - Radius, ScreenY2, 1, Color);
...
void OLED_Graph(uint8_t ScreenX, uint8_t ScreenY, uint8_t Width, uint8_t Height, uint8_t *Graph)
{
	...
	for (i = 0; i < Width; ++i)
	{
		...
		if (OLED_DisplayMode == Xor)
		{
			OLED_WritePage(i, j, Graph[index] >> YOffset);
		}
		index++;
	}
}
