...
void OLED_Shell_ShowDouble(uint8_t Line, uint8_t Column, double Number, uint8_t PreLength, uint8_t LastLength)
{
	if (Number >= 0)
	{
		OLED_Shell_ShowNum(Line, Column, Number, PreLength);
		OLED_Shell_ShowChar(Line, Column + PreLength, '.');
		OLED_Shell_ShowNum(Line, Column + PreLength + 1, (Number - (uint32_t)Number) * (double)OLED_Pow(10, LastLength), LastLength);
	}
	else
	{
		Number = -Number;
		OLED_Shell_ShowChar(Line, Column, '-');
		OLED_Shell_ShowNum(Line, 1 + Column, Number, PreLength);
		OLED_Shell_ShowChar(Line, 1 + Column + PreLength, '.');
		OLED_Shell_ShowNum(Line, 1 + Column + PreLength + 1, (Number - (uint32_t)Number) * (double)OLED_Pow(10, LastLength), LastLength);
	}
}
